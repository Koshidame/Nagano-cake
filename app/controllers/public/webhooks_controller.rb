class Public::WebhooksController < ApplicationController
  ...
  def create_order(session)
    Order.create!({
                    ...
                    address: session.shipping.address.state & session.shipping.address.line1 & session.shipping.address.line2, #多分エラー出る
                    name: session.shipping.name,
                    shipping_cost: session.shipping_options[0].shipping_amount,
                    status: '1',
                    total_payment: session.amount_total,
                    payment_method: session.payment_method_types,
                  })
  end

  def create_order_details(order, line_item)
    product = Stripe::Item.retrieve(line_item.price.product)
    purchased_product = Item.find(product.metadata.product_id)
